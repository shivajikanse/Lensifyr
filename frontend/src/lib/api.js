...
export const eventApi = {
  ...
  getOrganizerEvents: (organizerId) => api.get(`/event/organizer/${organizerId}`),
  verify: (eventCode) => api.post('/event/verify', { eventCode }),
};

export const imageApi = {
  upload: (formData, onProgress) => api.post('/image/upload', formData, {
  ...
  findMatches: (formData) => api.post('/image/find-matches', formData, {
    headers: { 'Content-Type': 'multipart/form-data' },
    responseType: 'blob',
  }),
  previewMatches: (formData) => api.post('/image/preview-matches', formData, {
    headers: { 'Content-Type': 'multipart/form-data' },
  }),
};
